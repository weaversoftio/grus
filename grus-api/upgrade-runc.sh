...
sleep 2
# try to remount /usr as read-only in a loop until it succeeds or 10 attempts are made
# keep trying until the remount succeeds          
while true; do
    echo "Remounting /usr as read-only..."
    sudo mount -o remount,ro /usr || { echo "Failed to remount /usr as read-only"; exit 1; }
    sleep 5
done

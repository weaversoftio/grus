...
async def create_registry_config(request: RegistryConfigRequest):
    ...
    if os.path.exists(path):
        return RegistryConfigResponse(
            success=False,
            message=f"Registry config file {request.name} already exists"
        )
    ...
    return RegistryConfigResponse(
        success=True,
        message=f"Registry config file {request.name} created successfully"
    )
